...
# estuaries
# all estuaries
estuaries = read.csv(list.files(pattern = "estuaries.csv", recursive = TRUE, full.names = TRUE))
...
estuaries = project(estuaries,pu)
...
aggregations = project(aggregations,pu)
...
values(mpa_layer_all)[which(values(mpa_layer_all)>=0.3)] = 1
...
# stack
lockedin = c(estuaries,aggregations,mpa_layer_all,mpa_layer_fullyprotected)
plot(lockedin)
