...
# sdm rasters
sdms_rasters = list.files(pattern = "Aseasonal_ensemblemean.tiff", recursive = TRUE, full.names =TRUE)
# thresholds
...
iucn_available = str_replace(iucn_available,"IUCN_layers/Sharks_rays_SA_raw/","")
...
for(i in 1:length(iucn_idx)){
  ...
  thresh_value = (thresh_value$cutoff)/1000
...
plot = levelplot(temp,
  ...
  # eez
    ...
    latticeExtra::layer(sp.points(places,col = "black",pch = 20))+
    ...
    latticeExtra::layer(sp.text(coordinates(places)[c(13:18),],places$Location[c(13:18)],col = "black",pch = 20,pos=2,cex = 0.5))+
    latticeExtra::layer(sp.text(coordinates(places)[c(9:10),],places$Location[c(9:10)],col = "black",pch = 20,pos=2,cex = 0.5))+
